...
export type AnyField = OrdinalPrimitive<any> | UniformArrayPrimitive<any> | UniformObjectPrimitive<any>

class OrdinalPrimitiveCtor<T extends RawPrimitives> implements OrdinalPrimitive<T> {
    kind: T
    constructor() { }
}
...
export class TObject<T extends AnyField> implements UniformObjectPrimitive<T> {
    ...
    constructor(ctor: TObject<T>['kind'], required = [],
        // isMap = false
    ) {
        this.kind = ctor
...
export class TArray<P extends AnyField> implements UniformArrayPrimitive<P['kind']> {
    kind: P[]
 ...
 */
type IfOptional<T extends Record<string, V>, R extends (keyof T)[], K extends keyof T, V extends AnyField> =
    K extends R[number] ? V : undefined;

type TUnwrapLower<T extends Record<string, AnyField>, R extends (keyof T)[]> = {
    [K in keyof T]?: (//
        T[K] extends OrdinalPrimitive<RawPrimitives> ? T[K]['kind'] : //
        T[K] extends TArray<AnyField> ? T[K]['pure'] : //
        T[K] extends TObject<T[K]['kind']> ? TUnwrapper<T[K]> : never //
    )
};
/**
 * Unwrap a(n) TObject
 */
export type TUnwrapper<T extends TObject<AnyField>> = TUnwrapLower<T['kind'], T['required']>;